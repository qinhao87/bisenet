import sys
sys.path.append(r'F:/py_pro/BiSeNet/img')
import argparse
...
def val(args, model, dataloader):
    ...
    with torch.no_grad():
        ...
        for i, (data, label) in enumerate(dataloader):
            ...
            # get gray predict image
            # import ipdb;ipdb.set_trace()
            predict = model(data).squeeze()
            predict = reverse_one_hot(predict)
            predict = predict.cpu().detach().numpy()

            # get gray label image
            label = label.squeeze()
            if args.loss == 'dice':
...
def train(args, model, optimizer, dataloader_train, dataloader_val):
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.validation_step == 0:
            precision = val(args, model, dataloader_val)
...
def main(params):
    ...
    train_path = os.path.join(args.data, 'train1')
    train_label_path = os.path.join(args.data, 'label1')
    test_path = os.path.join(args.data, 'train')
    test_label_path = os.path.join(args.data, 'label')
    dataset_train = crack(train_path, train_label_path,
    ...
    dataset_val = crack(test_path, test_label_path,
...
if __name__ == '__main__':
    params = [
        ...
        '--data', 'F:/py_pro/BiSeNet/few_img',
        '--num_workers', '0',
        '--num_classes', '2',
        '--cuda', '0',
        '--batch_size', '2',  # 6 for resnet101, 12 for resnet18
        '--save_model_path', 'F:/py_pro/BiSeNet/checkpoints_18_sgd',
